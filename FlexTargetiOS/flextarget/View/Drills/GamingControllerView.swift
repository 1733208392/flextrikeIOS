        ...
        ZStack {
            ...
            VStack(spacing: 30) {
                ...
                if !showResult {
                    ...
                    // Result View
                    VStack(spacing: 20) {
                        Text(NSLocalizedString("game_over", comment: "Game Over"))
                            .font(.title)
                            .foregroundColor(.white)
                        
                        HStack(spacing: 40) {
                            resultMetric(label: "SCORE", value: score)
                            resultMetric(label: "HIT", value: hitCount, color: .green)
                            resultMetric(label: "MISS", value: missCount, color: ftRed)
                        }
                        .padding(.vertical, 20)
                        ...
                        Button(action: {
                            ...
                            HStack {
                                ...
                                Text(NSLocalizedString("replay", comment: "Replay"))
                            }
                            .font(.headline)
                            .foregroundColor(.white)
                            .padding()
    ...
    private func setupResultListener() {
        ...
        NotificationCenter.default.addObserver(forName: .bleNetlinkForwardReceived, object: nil, queue: .main) { notification in
            guard let userInfo = notification.userInfo,
                  ...
                  let game = content["game"] as? String, game == "clay pigeon" else {
                return
            }
