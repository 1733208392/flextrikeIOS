...
var max_shots: int = 5  # Allow more shots to test all zones

# Reference to the IPSC Black target instance
@onready var ipsc_target = $IPSC
...
func _ready():
	...
	if ipsc_target and ipsc_target.has_signal("target_hit"):
		...
		print("Connected to IPSC Black target hit signal")
	else:
		print("ERROR: Could not connect to target hit signal")

func _on_target_hit(zone: String, points: int, hit_position: Vector2):
	"""Handle when the target is hit"""
	shot_count += 1
	print("IPSC Black target hit! Shot ", shot_count, "/", max_shots, " - Zone: ", zone, ", Points: ", points)
	
	# Display zone-specific messages
	match zone:
		"AZone":
			print("✓ A-Zone: Highest scoring zone!")
		"CZone":
			print("✓ C-Zone: Medium scoring zone!")
		"DZone":
			print("✓ D-Zone: Lowest scoring zone!")
		"BlackZoneLeft":
			print("✓ Black Zone Left: Invalid area - no points!")
		"BlackZoneRight":
			print("✓ Black Zone Right: Invalid area - no points!")
		"miss":
			print("✗ Miss: Outside all zones!")
...
func trigger_disappearing_animation():
	...
	if ipsc_target.has_method("play_disappearing_animation"):
		print("Triggering disappearing animation on IPSC Black target")
		ipsc_target.play_disappearing_animation()
	else:
		print("ERROR: play_disappearing_animation method not found on IPSC Black target")
...
func reset_target():
	...
	print("IPSC Black target reset for new test")
...
func remove_bullet_holes():
	...
	for child in ipsc_target.get_children():
		...
		if child.name.begins_with("BulletHole") or child.has_method("set_hole_position"):
			child.queue_free()
			print("Removed bullet hole: ", child.name)
...
func _input(event):
	...
	if event is InputEventKey and event.pressed:
		match event.keycode:
			KEY_R:
				reset_target()
				print("Target reset manually (R key)")
			KEY_T:
				trigger_disappearing_animation()
				print("Animation triggered manually (T key)")
			KEY_SPACE:
				print("Current shot count: ", shot_count, "/", max_shots)
			KEY_S:
				print("Current score: ", ipsc_target.get_total_score() if ipsc_target.has_method("get_total_score") else "N/A")