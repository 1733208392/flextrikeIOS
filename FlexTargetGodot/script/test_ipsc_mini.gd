...
# Reference to the IPSC target instance
...
func _ready():
	...
	if ipsc_target and ipsc_target.has_signal("target_hit"):
		...
		print("Connected to IPSC target hit signal")
	else:
...
func _on_target_hit(zone: String, points: int):
	...
	shot_count += 1
	print("Target hit! Shot ", shot_count, "/", max_shots, " - Zone: ", zone, ", Points: ", points)
...
func trigger_disappearing_animation():
	...
	if ipsc_target.has_method("play_disappearing_animation"):
		print("Triggering disappearing animation on IPSC target")
		ipsc_target.play_disappearing_animation()
	else:
		print("ERROR: play_disappearing_animation method not found on IPSC target")
...
func reset_target():
	...
	print("Target reset for new test")
