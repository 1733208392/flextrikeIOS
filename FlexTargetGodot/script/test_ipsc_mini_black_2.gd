...
# Reference to the IPSC Black 2 target instance
...
func _ready():
	...
	if ipsc_target and ipsc_target.has_signal("target_hit"):
		...
		print("Connected to IPSC Black 2 target hit signal")
	else:
...
func _on_target_hit(zone: String, points: int, hit_position: Vector2):
	...
	shot_count += 1
	print("IPSC Black 2 target hit! Shot ", shot_count, "/", max_shots, " - Zone: ", zone, ", Points: ", points)
	...
	match zone:
		...
		"BlackZone":
			print("✓ Black Zone: Invalid area - no points!")
		"miss":
...
func trigger_disappearing_animation():
	...
	if ipsc_target.has_method("play_disappearing_animation"):
		print("Triggering disappearing animation on IPSC Black 2 target")
		ipsc_target.play_disappearing_animation()
	else:
		print("ERROR: play_disappearing_animation method not found on IPSC Black 2 target")
...
func reset_target():
	...
	print("IPSC Black 2 target reset for new test")
