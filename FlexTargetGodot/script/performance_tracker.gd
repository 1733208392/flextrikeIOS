...
func _on_performance_saved(result, response_code, headers, body):
	if response_code == 200:
		...
		var http_service = get_node("/root/HttpService")
		if http_service:
			# Update max_index with circular buffer logic: cycle 1-15
			...
			if global_data and global_data.settings_dict != null:
				...
				if not DEBUG_DISABLED:
					print("[PerformanceTracker] Updated max_index from ", current_index, " to ", next_index, " (circular buffer 1-15)")
